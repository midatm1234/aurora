...
CASE_OUTPUT_DIR="outputs/O3_US-WEST_3day_lead_v4"
CASE_CHECKPOINT_DIR="outputs/checkpoints/O3_US-WEST_3day_lead_v4"
if [[ "${AURORA_ALLOW_CASE_OVERWRITE:-0}" != "1" ]] && \
   { [[ -e "$CASE_OUTPUT_DIR" ]] || [[ -e "$CASE_CHECKPOINT_DIR" ]]; }; then
    echo "Refusing to reuse existing O3 v4 artifacts:"
